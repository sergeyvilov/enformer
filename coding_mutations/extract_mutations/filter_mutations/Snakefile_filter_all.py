...
rule filter_calls:
    ...
    shell:
        r'''
        bcftools view {input.vcf} --max-alleles 2 -v "snps,indels" -i 'FILTER="PASS" && (CSQT~"NMD_transcript_variant" || CSQT~"stop_gained" || CSQT~"frameshift_variant" || CSQT~"splice_")'   -Oz -o {output.vcf}
        tabix -f {output.vcf}
        '''
